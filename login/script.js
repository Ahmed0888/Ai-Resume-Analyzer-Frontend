    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                setTimeout(() => {
                    // ✅ LOCAL DEV - Live Server pe dashboard
                    window.location.href = 'http://127.0.0.1:5501/public/dashboard/index.html   ';
                }, 1500);
            } else {
                showStatus(data.message || `Login failed (status ${response.status})`, 'error');
            }
        } catch (error) {
